...
public class CustomerService {
    ...
    // Validate OTP and check correctness
    public boolean validateOtp(String phoneNumber, String otp) {
        ...
        if (otpRecord.isPresent()) {
            ...
            if (passwordEncoder.matches(otp, storedOtp.getOtp()) && LocalDateTime.now().isBefore(storedOtp.getExpirationTime())) {
                // Update OTP verified state
                updateRegistrationState(phoneNumber, true, "name", null, null, null, null);  // Move to "name" step after OTP is verified
                return true;
            }
        }
        return false;
    ...
    // Retrieve the registration state from the database
    public Map<String, Object> getRegistrationState(String phoneNumber) {
        try {
            String sql = "SELECT otp_verified, step, name, surname, email, address FROM registration_state WHERE phone_number = ?";
            return jdbcTemplate.queryForMap(sql, phoneNumber);
        } catch (Exception e) {
            return null;  // Return null if no registration state is found
        }
    }

    // Update registration state, including the current step and user details
    public void updateRegistrationState(String phoneNumber, boolean otpVerified, String step, String name, String surname, String email, String address) {
        String sql = "INSERT INTO registration_state (phone_number, otp_verified, step, name, surname, email, address, updated_at) " +
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?) " +
                "ON CONFLICT (phone_number) DO UPDATE SET otp_verified = ?, step = ?, name = ?, surname = ?, email = ?, address = ?, updated_at = ?";

        jdbcTemplate.update(sql, phoneNumber, otpVerified, step, name, surname, email, address, LocalDateTime.now(),
                otpVerified, step, name, surname, email, address, LocalDateTime.now());
    }

    // Save registration data as it progresses
    public void saveRegistrationData(String phoneNumber, Customer newCustomer) {
        ...
        updateRegistrationState(phoneNumber, true, "complete", newCustomer.getName(), newCustomer.getSurname(), newCustomer.getEmail(), newCustomer.getAddress());
    }

    // Register customer in the database and clear their registration state
    public Customer registerCustomer(Customer customer) {
        customer.setCreatedAt(LocalDateTime.now());
        customerRepository.save(customer);
        removeRegistrationState(customer.getPhoneNumber());  // Clear state once registration is complete
        return customer;
    }

    // Remove registration state after successful registration
